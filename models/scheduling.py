...
class CogVideoXDPMScheduler_VTA(SchedulerMixin, ConfigMixin):
    """
    ...
    def add_noise(
        ...
        """
        original_samples: [B, F, C, H, W]
        noise:            [B, F, C, H, W]
        timesteps:        [B, F]  每个帧独立的 timestep
        """
        ...
        ### 扩展维度到 [B, F, 1, 1, 1] 方便广播
        # print("sqrt_alpha_prod shape before unsqueeze:", sqrt_alpha_prod.shape)
        # print("sqrt_one_minus_alpha_prod shape before unsqueeze:", sqrt_one_minus_alpha_prod.shape)
        # print("original_samples shape:", original_samples.shape)
        # sqrt_alpha_prod shape before unsqueeze: torch.Size([2, 2])                                                                                                                                                                          
        # sqrt_one_minus_alpha_prod shape before unsqueeze: torch.Size([2, 2])                                      
        # original_samples shape: torch.Size([2, 2, 4, 8, 8])  
        # assert 0

        original_samples_shape = original_samples.shape
    ...
    def get_velocity(self, sample: torch.Tensor, noise: torch.Tensor, timesteps: torch.IntTensor) -> torch.Tensor:
        """
        sample: torch.Tensor,      # [B, F, C, H, W] 或 [B, C, H, W]
        noise: torch.Tensor,       # 同 shape
        timesteps: torch.IntTensor # scalar / [B] / [B, F]
        计算 velocity, 支持 scalar / 1D / 2D timesteps。
        """
