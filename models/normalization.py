...
class AdaLayerNorm_VTA(nn.Module):
    r"""
    ...
    def forward(
        ...
        temb = self.linear(self.silu(temb))  # 假设输出 shape: [B, C] 或 [B, T, C]

        if temb.dim() == 3:
            # 3D temb: [B, T, C]
            if self.chunk_dim == 1:
                # 按最后一个维度拆分，dim=2
                shift, scale = temb.chunk(2, dim=-1)  # 两半各[C/2]
            else:
                assert 0, "chunk_dim=0 not supported for 3D temb"

        elif temb.dim() == 2:
            # 2D temb: [B, C]
            if self.chunk_dim == 1:
                shift, scale = temb.chunk(2, dim=1)  # 按最后一维拆分
                shift = shift[:, None, :]  # 增加时间维度1
                scale = scale[:, None, :]
            else:
                scale, shift = temb.chunk(2, dim=0)
        else:
            raise ValueError(f"Unsupported temb dimension: {temb.dim()}")
...
class CogVideoXLayerNormZero_VTA(nn.Module):
    def __init__(
        self,
    ...
    def forward(
        ...
        if is_3d:
            B, T, F = temb.shape
            temb = temb.reshape(B * T, F)

        shift, scale, gate, enc_shift, enc_scale, enc_gate = self.linear(self.silu(temb)).chunk(6, dim=1)

        if is_3d:
            # reshape 回 [B, T, C]
            shift      = shift.reshape(B, T, -1)
            scale      = scale.reshape(B, T, -1)
            gate       = gate.reshape(B, T, -1)
            enc_shift  = enc_shift.reshape(B, T, -1)
            enc_scale  = enc_scale.reshape(B, T, -1)
            enc_gate   = enc_gate.reshape(B, T, -1)

            # 如果 T < hidden_states.shape[1]，进行时间维 broadcast
            target_T = hidden_states.shape[1]
            enc_target_T = encoder_hidden_states.shape[1]
            print(f"target_T: {target_T}, T: {T}, hidden_states.shape: {hidden_states.shape}, encoder_hidden_states.shape: {encoder_hidden_states.shape}")
            # target_T: 9450, T: 14, hidden_states.shape: torch.Size([4, 9450, 3072]), encoder_hidden_states.shape: torch.Size([4, 226, 3072])
            if T != target_T:
                # 这里用 repeat_interleave 保证在时间维重复
                repeat_factor = target_T // T
                enc_repeat_factor = enc_target_T // T
                if target_T % T != 0 or enc_target_T % T != 0:
                    raise ValueError(f"Cannot broadcast temb from T={T} to target_T={target_T} evenly or enc_target_T={enc_target_T} evenly")
                ...
                enc_shift  = enc_shift.repeat_interleave(enc_repeat_factor, dim=1)
                enc_scale  = enc_scale.repeat_interleave(enc_repeat_factor, dim=1)
                enc_gate   = enc_gate.repeat_interleave(enc_repeat_factor, dim=1)

                print(f"Broadcasting temb from T={T} to target_T={target_T} by repeating {repeat_factor} times")
                # Broadcasting temb from T=14 to target_T=9450 by repeating 675 times 
                print(f"shift shape: {shift.shape}, scale shape: {scale.shape}, gate shape: {gate.shape}")
                # shift shape: torch.Size([4, 9450, 3072]), scale shape: torch.Size([4, 9450, 3072]), gate shape: torch.Size([4, 9450, 3072]) 
                print(f"enc_shift shape: {enc_shift.shape}, enc_scale shape: {enc_scale.shape}, enc_gate shape: {enc_gate.shape}")
            ...
            return hidden_states, encoder_hidden_states, gate, enc_gate  # 3D本身已经有T维，无需加None
